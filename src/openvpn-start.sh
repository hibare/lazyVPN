...
checkError() {
		if [ $? -ne 0 ]
		then
			echo "${YELLOW}[ $skullMark ] $2 ${NC}"
			eval lastArg=\${$#}
			if [ "$lastArg" = "exit"]
			then
				echo "${RED}[ $skullMark ] Abort${NC}"
			fi
		else
...
then
	echo "${GREEN}[ $checkMark ] Tunnel available... ${NC}"
	exit 0
else
	#Get password
	echo "${CYAN}[ $arrowMark ] Getting password ...${NC}"
	password=`curl -s https://www.vpnbook.com/freevpn | grep "Password" | head -n 1 | awk '{print $2}' | cut -c "9-" | cut -d "<" -f 1`
	#check if password is fetched
	if [ "x$password" = "x" ]
	then
		echo "${RED}[ $skullMark ] Abort ${NC}"
		exit
	else
		echo "${GREEN}[ $checkMark ] Password fetched ${NC}"
		echo "${CYAN}[ $arrowMark ] Current password : $password ${NC}"
	fi
	
	#Open tunnel
	echo "${CYAN}[ $arrowMark ] Opening tunnel ... ${NC}"
	
	cd $HOME/.crazyScripts/openVPN

	expect -c '
	 spawn openvpn --config vpnbook-euro2-tcp443.ovpn
	...
	if pgrep openvpn > /dev/null
	then 
		echo "${GREEN}[ $checkMark ] Tunnel established... Enjoy your privacy...!!!${NC}"
	else
		echo "${RED}[ $skullMark ] Tunnel collapsed! ${NC}"
	fi
	exit 0
fi
